...
void ImageSettings::applyImageSettings()
{
    qDebug() << "Applying image settings";
    ...
    QSize oldResolution = imagecapture->resolution();
    QSize newResolution = boxValue(ui->imageResolutionBox).toSize();
    imagecapture->setResolution(newResolution);


    if (oldResolution != newResolution) {

        qDebug() << "Resolution changed from" << oldResolution << "to" << newResolution;
    }
}
